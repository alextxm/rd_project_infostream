    ...
    class Program
    {
        ...
        static void ManageIndex(IElasticClient client, ConnectionSettings settings, bool forceIndexRecreation)
        {
            ...
            if (forceIndexRecreation)
            {
                ...
                // 4) a questo punto usa il MapFluent per fare l'override
                // verifica su: http://localhost:9200/indexname/typename/_mapping?pretty
                // Aggiornamento: 28/02/2014 00:15
                // questo non va piu'bene: in ES 1.0 se distruggi un mapping elimini dal cluster anche tutti i documenti associati
                client.CreateIndex(indexname); //, new IndexSettings());
            }
            ...
            if (map == null)
            {
                ...
                mapper.MapToElasticSearch(indexname, client);
                //map = client.GetMapping<EmailMessage>(p => p.Index(indexname)).Mapping;
            }
        ...
        static void Main(string[] args)
        {
            ...
            {
                long totItems = client.IndicesStats(p => p.Index(indexname)).Stats.Total.Documents.Count;

                bool usescoring = cmdline.Options.IsOptionEnabled("withscore");
                ...
                Console.WriteLine("Found {1} items matching on {0} total; duration: query={2}ms, full={3}ms", totItems, tz.Hits.Count(), tz.ElapsedMilliseconds, (int)(t2 - t1).TotalMilliseconds);

                foreach (IHit<EmailMessage> r in tz.Hits)
                {
