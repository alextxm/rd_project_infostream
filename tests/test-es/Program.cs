    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                IHealthResponse h = client.ClusterHealth();
            ...
            if (doReindex)
            {
...
#if USE_PLAINEN
#else
                    if (client.IndexExists(p => p.Index(indexname)).Exists)
                        client.DeleteIndex(p => p.Index(indexname));
                    
                    client.CreateIndex(indexname); // (indexname, new IndexSettings());
#endif
                }

#if !BOH
                RootObjectMapping map = client.GetMapping<StaticDataFeed>(p => p.Index(indexname)).Mapping;
                if (map != null)
                    client.DeleteMapping<StaticDataFeed>(p => p.Index(indexname));
                ...
                foreach (StaticDataFeed f in data)
                {
                    ...
                    if (updateindex)
                    {
                        ...
                        client.Index<StaticDataFeed>(f, p => p.Index(indexname).Type("staticdatafeed2")); // indexname, "staticdatafeed2");
#endif
            ...
            {
                RootObjectMapping ex = client.GetMapping(p => p.Index(indexname).Type("staticdatafeed2")).Mapping;
                if (ex == null)
                {
...
#else
                var tz0 = client.Count(p => p.Index(indexname)); // (new[] { indexname }, p => p.MatchAll());
